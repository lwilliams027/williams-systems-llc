...
document.querySelectorAll('[data-journey]').forEach((section, idx) => {
  ...
  scenes.forEach((scene, i) => {
    ...
    mark(i, T + 0.001);
    tl.to($$('.sj-copy > *', scene), { autoAlpha: 1, y: 0, duration: 0.3, stagger: 0.07, ease: 'power3.out' }, T + 0.12);

    // the animations inside this scene
    $$('[data-sj]', scene).forEach((el) => {
      const at = T + 0.3 + (parseFloat(el.dataset.d) || 0);
      const kind = el.dataset.sj;
      if (kind === 'type') {
        const txt = el.dataset.text, p = { n: 0 };
        el.textContent = '';
        tl.to(p, { n: txt.length, duration: Math.min(0.5, 0.05 + txt.length * 0.012), onUpdate: () => { el.textContent = txt.slice(0, Math.round(p.n)); } }, at);
      } else if (kind === 'flap') {
        el.textContent = el.dataset.text.replace(/[A-Z0-9]/g, ' ');
        tl.to(el, { scrambleText: { text: el.dataset.text, chars: LETTERS, speed: 0.6, revealDelay: 0.3 }, duration: 0.35 }, at);
        if (el.dataset.color) tl.fromTo(el, { color: '#9CA3AF' }, { color: el.dataset.color, duration: 0.05 }, at + 0.33);
      } else if (kind === 'draw') {
        const paths = el.matches('path, line, circle, rect, polyline') ? [el] : $$('path, line, circle, polyline', el).filter((p) => !p.closest('defs'));
        gsap.set(paths, { drawSVG: '0%' });
        tl.to(paths, { drawSVG: '100%', duration: parseFloat(el.dataset.dur) || 0.6, stagger: 0.06, ease: 'power1.inOut' }, at);
      } else if (kind === 'flip') {
        const tiles = $$('.sj-tile-in', el);
        gsap.set(tiles, { rotateY: 180 });
        tl.to(tiles, { rotateY: 0, duration: 0.35, stagger: 0.09, ease: 'back.out(1.4)' }, at);
      } else if (kind === 'pop') {
        gsap.set(el, { autoAlpha: 0, scale: 0.6 });
        tl.to(el, { autoAlpha: 1, scale: 1, duration: 0.2, ease: 'back.out(2)' }, at);
      } else if (kind === 'stamp') {
        gsap.set(el, { autoAlpha: 0, scale: 2.2, rotate: -18 });
        tl.to(el, { autoAlpha: 1, scale: 1, rotate: -8, duration: 0.18, ease: 'power4.in' }, at);
      } else if (kind === 'count') {
        const end = +el.dataset.count, v = { n: 0 };
        el.textContent = fmt(el, 0);
        tl.to(v, { n: end, duration: 0.5, ease: 'power2.out', onUpdate: () => { el.textContent = fmt(el, v.n); } }, at);
      } else if (kind === 'check') {
        $$('li', el).forEach((li, k) => {
          const p = { v: 0 };
          tl.to(p, { v: 1, duration: 0.05, onUpdate: () => li.classList.toggle('done', p.v > 0.5) }, at + k * 0.1);
        });
      } else if (kind === 'rise') {
        gsap.set(el.children, { autoAlpha: 0, y: 30 });
        tl.to(el.children, { autoAlpha: 1, y: 0, duration: 0.25, stagger: 0.08, ease: 'power3.out' }, at);
      } else if (kind === 'grow') {
        gsap.set(el.children, { scaleY: 0, transformOrigin: '50% 100%' });
        tl.to(el.children, { scaleY: 1, duration: 0.35, stagger: 0.04, ease: 'power3.out' }, at);
      } else if (kind === 'fill') {
        gsap.set(el, { scaleX: 0, transformOrigin: '0% 50%' });
        tl.to(el, { scaleX: 1, duration: parseFloat(el.dataset.dur) || 0.5, ease: 'power1.inOut' }, at);
      }
    });
  });
