...
// page → [motion style (see STYLES in src/js/flip-journey.js), glow tint, flip order, finale board label]; every page gets its own
const PAGES = {
  'web-apps':             ['cube', '99,102,241', ['y', 'x', '-y', 'zoom', '-x', 'y', 'x'], 'YOUR WEB APP'],
  'saas':                 ['deck', '16,185,129', ['x', '-y', 'zoom', 'y', '-x', 'x', 'y'], 'YOUR SAAS'],
  'cloud':                ['cloud', '56,189,248', ['-y', 'zoom', 'x', '-x', 'y', 'zoom', 'x'], 'YOUR CLOUD SETUP'],
  'personalized-ai':      ['scan', '160,107,255', ['zoom', 'y', '-x', 'x', '-y', 'y', 'zoom'], 'YOUR AI ASSISTANT'],
  'launch-a-new-product': ['launch', '76,175,80', ['x', 'y', 'zoom', '-y', 'x', 'y'], 'YOUR PRODUCT'],
  'modernize-an-app':     ['wipe', '236,72,153', ['-x', 'y', 'x', 'zoom', '-y', 'x'], 'YOUR APP'],
  'replace-spreadsheets': ['fold', '20,184,166', ['y', '-x', 'zoom', 'x', '-y', 'y'], 'YOUR NEW TOOL'],
  'secure-your-software': ['iris', '239,68,68', ['zoom', '-y', 'x', 'y', '-x', 'zoom'], 'YOUR SOFTWARE'],
  'move-to-the-cloud':    ['migrate', '14,165,233', ['-y', 'x', 'y', 'zoom', '-x', 'x'], 'YOUR SERVERS'],
  'ongoing-support':      ['swing', '59,130,246', ['x', 'zoom', '-y', 'y', '-x', 'x'], 'YOUR SOFTWARE'],
};
...
function journey(slug, d, [style, tint, flips, label]) {
  const scenes = [];
  ...
  d.chapters.forEach((c, i) => {
    scenes.push(`        <div class="sj-scene" data-scene data-label="${c.k.replace(/<[^>]+>/g, '')}" data-flip="${flips[i % flips.length]}">
          <div class="sj-copy">
  ...
  return `    <!-- ============ ${d.product} journey ============
    ...
    <section class="sj sj-product" data-journey data-style="${style}" style="--sj-tint:${tint}" aria-labelledby="pageTitle">
      <div class="sj-stage">
